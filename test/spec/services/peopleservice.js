...
describe('Service: peopleService', function () {
  ...
  describe('Adding a Person', function(){
    ...
    it('should give the new person the highest uuid', function(){
      peopleService.addPerson(newPerson);
      var personAdded = _.last(peopleService.people);
      var maxUuidPerson = _.max(peopleService.people, function(person){ return person.uuid } );
      expect(personAdded).toBe(maxUuidPerson);
    });
